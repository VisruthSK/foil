...
fn process_runner_overhead() -> Result<()> {
    ...
    eprintln!("work_ms,runs,mean_us,p50_us,p90_us,p99_us,min_us,max_us,negative");
    ...
    for work_ms in [0_u64, 1, 10, 100] {
        ...
        for order in schedule {
            ...
            for side in order.sides() {
                match side {
                    ...
                    Side::Candidate => {
                        ...
                        let run = command.run_in(directory.path(), "overhead benchmark")?;
                        foil = Some((
            ...
            overhead.push(
                ((foil_external - foil_internal) - (raw_external - raw_internal)) as f64 / 1_000.0,
            );
        }
        ...
        eprintln!(
            "{work_ms},{runs},{mean:.3},{:.3},{:.3},{:.3},{:.3},{:.3},{negative}",
